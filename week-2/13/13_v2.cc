...
int main(int argc, char* argv[])
{
	...
	string calculation;				// initialize variable to save calculation while
	                                // printing binary digits

	int remainingBinaryDigits = 32;			// we need to print 32 binary digits
	                                // -> keep track of how many already printed

	if (number < 0)						// if it's a negative number
	{
		int const firstBitNegative = -pow(2, 31);		// take -(2^31)
	...
	{
		int const exponent = log2(number);			        // get the exponent to compute the
		                                                    // nearest power of two to the number

		int const nearestPowerOfTwo = pow(2, exponent);	// get nearest power of two

		while (remainingBinaryDigits > exponent + 1)	// print 0's until the point where a 1
		                                                // should be printed for the nearest power of two
		{
